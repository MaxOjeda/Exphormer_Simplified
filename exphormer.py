...
class ExphormerAttention(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, num_heads, use_bias, dim_edge=None,
                 ...
                 use_distmult_v=False, use_rel_matrix_v=False):
        super().__init__()
        ...
        # Full relation-specific matrix: msg(i→j, r) = h_i @ W_r[r_uv]
        # One (out_dim × out_dim) matrix per relation per head — NBFNet-exact expressivity.
        # Replaces both W_V (shared projection) and V_gate (diagonal scaling).
        # With dim=64, 18 rels: 18 × 4 × 16 × 16 = 18,432 params.
        self.use_rel_matrix_v = use_rel_matrix_v
        if use_rel_matrix_v:
            assert num_relations is not None, "use_rel_matrix_v requires num_relations"
            assert not use_edge_gating, "use_rel_matrix_v and use_edge_gating are mutually exclusive"
            assert not use_nbf_v, "use_rel_matrix_v and use_nbf_v are mutually exclusive"
            assert not use_distmult_v, "use_rel_matrix_v and use_distmult_v are mutually exclusive"
            # +1 for expander edge sentinel (index = num_relations)
            self.W_r = nn.Parameter(
                torch.empty(num_relations + 1, num_heads, self.out_dim, self.out_dim)
            )
            # Orthogonal init per (relation, head): preserves norms at init, good gradient flow.
            for r in range(num_relations + 1):
                for head in range(num_heads):
                    nn.init.orthogonal_(self.W_r.data[r, head])
        ...
        if not use_nbf_v and not use_rel_matrix_v:
        ...
        if use_query_conditioning:
            ...
            if use_edge_gating or use_rel_matrix_v:
    ...
    def propagate_attention(self, batch, edge_index):
        ...
        # Value for each edge.
        if self.use_rel_matrix_v:
            # Full relation-specific matrix: msg(i→j, r) = h_i @ W_r[r_uv]
            # Memory-efficient: iterate over relation types (19 for WN18RR) instead of
            # materializing a (E, heads, d, d) tensor for all edges simultaneously.
            rel_ids = batch.edge_rel_idx.to(torch.long)                          # (E,)
            v_raw = batch.V_h[edge_index[0].to(torch.long)]                      # (E, heads, out_dim)
            v_src = torch.zeros_like(v_raw)
            for r in range(self.W_r.shape[0]):
                mask = rel_ids == r
                if not mask.any():
                    continue
                # einsum over E_r edges of this relation only — no large tensor materialized
                v_src[mask] = torch.einsum('ehk,hkd->ehd', v_raw[mask], self.W_r[r])
            # Query conditioning: msg = (h @ W_r[r_uv]) * (1 + proj_vg(r_q))
            # Recovers the V-query signal that was present in the baseline gate.
            if hasattr(batch, 'E_relmat_qcond'):
                v_src = v_src * (1.0 + batch.E_relmat_qcond)
        elif self.use_distmult_v:
            # DistMult-inside-attention: msg(i→j,r) = W_V(h_i) ⊙ W_r
    ...
    def forward(self, batch):
        ...
        elif self.use_nbf_v or self.use_rel_matrix_v:
            V_h = h                        # raw h; W_r/nbf_rel applied per-edge in propagate_attention
        else:
        ...
        if self.use_rel_matrix_v and shared_edge is not None:
            # Query conditioning for W_r output: msg = (h @ W_r[r_uv]) * (1 + proj_vg(r_q))
            # proj_vg is the same projection used by the edge gate — reused here for V-query conditioning.
            batch.E_relmat_qcond = self.proj_vg(shared_edge).view(-1, self.num_heads, self.out_dim)
